...
- `lsp/` — one LSP entry declared in `.claude-plugin/plugin.json`'s `lspServers`, pointing at `lsp/proxy/run.sh`. The wrapper templates `${CLAUDE_PLUGIN_ROOT}` into a generated config and execs upstream `lsp-proxy.py` (a user-provided prerequisite in `~/.cache/safer-by-default/`; see INSTALL.md — not auto-installed by `/safer:setup` yet). The proxy fans out to `typescript-language-server` (primary, for code intelligence — `documentSymbol`, `goToDefinition`, `findReferences`, `hover`) and `lsp/architecture/server/index.ts` (sidecar, custom Effect-shaped analyzer that fires architecture diagnostics with `codeDescription.href` links to `PRINCIPLES.md`). ESLint syntax rules live on the CLI surface that `/safer:setup` wires into each project, gated by `/safer:verify`. See `ARCHITECTURE.md` → LSP integration.
