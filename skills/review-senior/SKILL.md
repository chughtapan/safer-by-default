...
- URL matches `/issues/\d+#issuecomment-` → read the comment's `safer:*`
  label on the parent issue. `safer:requirements` → `contract`; `safer:architect`
...
- **"I'll rewrite the routing table to cover a new case I just noticed."**
  No. The routing table lives in SPEC r4.1 §5(h). Route to `/safer:requirements`
...
`STATUS: <marker>. Artifact: <URL>. Aggregate verdict: <APPROVE|REQUEST-CHANGES|HOLD>. Process issues: <none | one-line list>.`
