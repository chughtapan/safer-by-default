...
For each composed gstack skill in the row determined by Phase 1, dispatch via the `Skill` tool in order, collecting verdicts (APPROVE / REQUEST-CHANGES / HOLD / COMMENT) as each returns:

```
Skill({ skill: "review",  args: "--pr <PR-URL> --hold-scope" })
Skill({ skill: "simplify", args: "--pr <PR-URL> --hold-scope" })
Skill({ skill: "codex",    args: "--mode review --artifact <PR-URL> --hold-scope" })
```

Substitute the row's actual skill list. `--hold-scope` is mandatory: composed gstack skills cannot prompt the user mid-run from inside review-senior; if they would, they escalate to `/safer:orchestrate` which surfaces the prompt via `AskUserQuestion`.
