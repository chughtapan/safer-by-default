...
**Maintenance note:** This mapping is hand-maintained. It is updated when and only when `skills/setup/SKILL.md`'s `eslint-plugin-agent-code-guard@<spec>` install line changes. There is no CI drift gate; upstream ACG releases not yet reflected in the install line are informational only.

### Rule mapping

The mapping below enumerates every rule exported by the version of `eslint-plugin-agent-code-guard` that `skills/setup/SKILL.md` currently installs: `eslint-plugin-agent-code-guard@^0.0.2` (resolved: `0.0.2`). All 13 rules are listed.

| Rule | Principle | Rationale | Severity | Preset |
|---|---|---|---|---|
| `async-keyword` | [principle 3 — Errors are typed, not thrown](../../PRINCIPLES.md#3-errors-are-typed-not-thrown--tagged-errors-or-typed-results-no-raw-throws-no-silent-catches) | In Effect-first code, all asynchrony flows through Effect; `Promise<T>` erases the error channel. [Before/After (local)](node_modules/eslint-plugin-agent-code-guard/docs/rules/async-keyword.md) / [GitHub](https://github.com/chughtapan/agent-code-guard/blob/v0.0.2/docs/rules/async-keyword.md) | error | recommended |
| `promise-type` | [principle 3 — Errors are typed, not thrown](../../PRINCIPLES.md#3-errors-are-typed-not-thrown--tagged-errors-or-typed-results-no-raw-throws-no-silent-catches) | Function return types should declare `Effect<A, E, R>`, not `Promise<A>`; Promise erases the error and requirements channels. [Before/After (local)](node_modules/eslint-plugin-agent-code-guard/docs/rules/promise-type.md) / [GitHub](https://github.com/chughtapan/agent-code-guard/blob/v0.0.2/docs/rules/promise-type.md) | error | recommended |
| `then-chain` | [principle 3 — Errors are typed, not thrown](../../PRINCIPLES.md#3-errors-are-typed-not-thrown--tagged-errors-or-typed-results-no-raw-throws-no-silent-catches) | `.then()` chains return to untyped-error promise-land; use `Effect.flatMap` / `Effect.map` or wrap at the boundary once. [Before/After (local)](node_modules/eslint-plugin-agent-code-guard/docs/rules/then-chain.md) / [GitHub](https://github.com/chughtapan/agent-code-guard/blob/v0.0.2/docs/rules/then-chain.md) | error | recommended |
| `bare-catch` | [principle 3 — Errors are typed, not thrown](../../PRINCIPLES.md#3-errors-are-typed-not-thrown--tagged-errors-or-typed-results-no-raw-throws-no-silent-catches); also [principle 4 — Exhaustiveness over optionality](../../PRINCIPLES.md#4-exhaustiveness-over-optionality--every-branch-handled-switches-end-in-never) | Silent catch (`catch {}` or `catch (_)`) hides both the error and the branch; exhaustiveness cannot apply. On Effect, use `Effect.try` / `Effect.tryPromise` with explicit catch. [Before/After (local)](node_modules/eslint-plugin-agent-code-guard/docs/rules/bare-catch.md) / [GitHub](https://github.com/chughtapan/agent-code-guard/blob/v0.0.2/docs/rules/bare-catch.md) | error | recommended |
| `record-cast` | [principle 2 — Validate at every boundary](../../PRINCIPLES.md#2-validate-at-every-boundary--schemas-where-data-enters-types-inside) | `as Record<string, unknown>` papers over a missing schema at the edge. Decode into a typed result instead using Effect Schema, Zod, or your own parser. [Before/After (local)](node_modules/eslint-plugin-agent-code-guard/docs/rules/record-cast.md) / [GitHub](https://github.com/chughtapan/agent-code-guard/blob/v0.0.2/docs/rules/record-cast.md) | error | recommended |
| `no-raw-sql` | [principle 1 — Types beat tests](../../PRINCIPLES.md#1-types-beat-tests--move-constraints-into-the-type-system); also [principle 2 — Validate at every boundary](../../PRINCIPLES.md#2-validate-at-every-boundary--schemas-where-data-enters-types-inside) | Raw SQL defeats the compiler; use a typed builder (Kysely, Drizzle, or similar) to make the schema load-bearing. [Before/After (local)](node_modules/eslint-plugin-agent-code-guard/docs/rules/no-raw-sql.md) / [GitHub](https://github.com/chughtapan/agent-code-guard/blob/v0.0.2/docs/rules/no-raw-sql.md) | error | recommended |
| `no-manual-enum-cast` | [principle 1 — Types beat tests](../../PRINCIPLES.md#1-types-beat-tests--move-constraints-into-the-type-system); also [principle 2 — Validate at every boundary](../../PRINCIPLES.md#2-validate-at-every-boundary--schemas-where-data-enters-types-inside) | Hand-written unions drift; import generated or schema-derived unions instead to keep the type system a truth-teller. [Before/After (local)](node_modules/eslint-plugin-agent-code-guard/docs/rules/no-manual-enum-cast.md) / [GitHub](https://github.com/chughtapan/agent-code-guard/blob/v0.0.2/docs/rules/no-manual-enum-cast.md) | error | recommended |
| `no-hardcoded-secrets` | [principle 2 — Validate at every boundary](../../PRINCIPLES.md#2-validate-at-every-boundary--schemas-where-data-enters-types-inside) | Hardcoded secrets bypass the environment-boundary schema entirely; load from environment through a validated schema. [Before/After (local)](node_modules/eslint-plugin-agent-code-guard/docs/rules/no-hardcoded-secrets.md) / [GitHub](https://github.com/chughtapan/agent-code-guard/blob/v0.0.2/docs/rules/no-hardcoded-secrets.md) | error | recommended |
| `no-raw-throw-new-error` | [principle 3 — Errors are typed, not thrown](../../PRINCIPLES.md#3-errors-are-typed-not-thrown--tagged-errors-or-typed-results-no-raw-throws-no-silent-catches) | Raw `throw new Error(...)` erases type information; use tagged error classes or discriminated-union results instead. [Before/After (local)](node_modules/eslint-plugin-agent-code-guard/docs/rules/no-raw-throw-new-error.md) / [GitHub](https://github.com/chughtapan/agent-code-guard/blob/v0.0.2/docs/rules/no-raw-throw-new-error.md) | error | recommended |
| `no-test-skip-only` | [principle 1 — Types beat tests](../../PRINCIPLES.md#1-types-beat-tests--move-constraints-into-the-type-system) (test-hygiene corollary) | A test that does not run in CI is decoration; `skip` and `only` are debugging tools, not code. Per Principle 1 corollary item 4: if tests exist, CI runs them. [Before/After (local)](node_modules/eslint-plugin-agent-code-guard/docs/rules/no-test-skip-only.md) / [GitHub](https://github.com/chughtapan/agent-code-guard/blob/v0.0.2/docs/rules/no-test-skip-only.md) | error | recommended |
| `no-coverage-threshold-gate` | [principle 1 — Types beat tests](../../PRINCIPLES.md#1-types-beat-tests--move-constraints-into-the-type-system) (test-hygiene corollary) | Coverage percentage is a diagnostic, never a CI gate; gating on 80% coverage is Goodhart-broken (Inozemtseva & Holmes ICSE 2014). Per Principle 1 corollary item 2: act on the cause of a gap (dead code, missing path), not on the number. [Before/After (local)](node_modules/eslint-plugin-agent-code-guard/docs/rules/no-coverage-threshold-gate.md) / [GitHub](https://github.com/chughtapan/agent-code-guard/blob/v0.0.2/docs/rules/no-coverage-threshold-gate.md) | warn | recommended |
| `no-hardcoded-assertion-literals` | [principle 1 — Types beat tests](../../PRINCIPLES.md#1-types-beat-tests--move-constraints-into-the-type-system) (test-hygiene corollary) | Magic assertion values are silent metadata; name them as constants. Per Principle 1 corollary item 1: a compressed assertion is a compression of information with lower coverage. [Before/After (local)](node_modules/eslint-plugin-agent-code-guard/docs/rules/no-hardcoded-assertion-literals.md) / [GitHub](https://github.com/chughtapan/agent-code-guard/blob/v0.0.2/docs/rules/no-hardcoded-assertion-literals.md) | warn | recommended |
| `no-vitest-mocks` | [principle 2 — Validate at every boundary](../../PRINCIPLES.md#2-validate-at-every-boundary--schemas-where-data-enters-types-inside) | Integration tests that mock the database assert that your code works against your mock, not the real dependency; use `testcontainers` or real test-mode services. The mock is a fiction at the boundary. [Before/After (local)](node_modules/eslint-plugin-agent-code-guard/docs/rules/no-vitest-mocks.md) / [GitHub](https://github.com/chughtapan/agent-code-guard/blob/v0.0.2/docs/rules/no-vitest-mocks.md) | error | integrationTests |

Each rule ships a full `Before` / `After` example. The local path `node_modules/eslint-plugin-agent-code-guard/docs/rules/<rule-name>.md` is canonical for reproducibility at the install-line baseline. The GitHub URL is a convenience link for reviewers.
