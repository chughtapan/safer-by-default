...
#
# Two install shapes count, and this predicate must stay identical to the one in
# ./setup at the repo root. A directory-only test rejects a marketplace plugin
# install, so the two entry points would disagree about whether gstack is
# present: the user passes one and fails the other with no way to tell why.
_gstack_present() {
  [ -d "$HOME/.claude/skills/gstack" ] && return 0
  for s in "$HOME/.claude/settings.json" "$HOME/.claude/settings.local.json"; do
    [ -f "$s" ] && grep -q '"gstack@[^"]*"[[:space:]]*:[[:space:]]*true' "$s" && return 0
  done
  return 1
}
if ! _gstack_present; then
  cat <<'EOF'
ERROR: gstack is required but not found.
Looked for: ~/.claude/skills/gstack/ and an enabled gstack plugin in settings.json.
...
- **Brown-field path:** produces a phased migration plan and ratchets to `/safer:requirements → /safer:architect → /safer:implement-*` for any code edits. Those modalities are downstream destinations. The skill itself does NOT mass-edit legacy code (Principle 6 + Principle 8 enforcement).
