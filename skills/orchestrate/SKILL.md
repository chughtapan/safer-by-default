...
- The modality skills exist in the plugin (`/safer:requirements`, `/safer:architect`, etc.).
...
|---|---|---|---|
| `requirements` | goals, non-goals, acceptance, invariants | architecture, libs, code | any structural commitment |
...
Four worked contracts live in `docs/contracts/`, one per dispatch shape that recurs: `invitation.md` (dialog only), `bug-fix-end-to-end.md` (diagnose through merge), `scrum-master-backlog.md` (work an existing epic), `architect-and-stop.md` (plan, then stop before implementing). Each is a real `## Autonomy contract` block, not a description of one. Read the shape closest to the user's instruction and adapt it rather than drafting from scratch; `docs/contracts/README.md` maps instruction to shape.
...
**Confidence gate.** If parsing leaves you unsure on any of the five fields, present 2–3 candidate contract shapes to the user via `AskUserQuestion`, recommended-default first. Do NOT silently pick a shape; the asymmetric cost framing favors over-asking. A wrong autonomous draft burns trust; a wrong question costs one reply.
...
On `OK`, the contract becomes the parent epic's `## Autonomy contract` section (Phase 3) and the orchestrator continues to Phase 2.
...
**Mode.** <feature-ship | refactor | burndown, from Phase 1a draft>
...
|---|---|---|---|---|
| 1 | requirements | — | Contract published as a comment on this epic; goals, non-goals, and explicit acceptance criteria present; `safer:requirements` label; state `review` | <https://github.com/OWNER/REPO/issues/NNN> |
...
- `requirements` / `architect` → `plan-approved` → (next sub-task starts, this one closes to `done`).
...
```bash
gh issue view "$PARENT" --json body --jq '.body' | awk '/^## (Autonomy contract|Contract)$/{f=1;print;next} f&&/^## /{f=0} f{print}' > /tmp/contract.md
```
...
The next dispatch is outside the parent epic's `## Autonomy contract` autonomy budget.
...
Dispatching more than one teammate? Read `skills/orchestrate/references/auto-monitor.md` and install the loop **before** the first dispatch. Skipping it is a stop-rule violation.
...
3. **Diff review mode (implement-staff mandatory; implement-senior optional):** codex reads the PR diff, independent of `/safer:review-senior`. Verdict posted as a PR comment before the human review fires. Counts as one independent pass toward the stamina N budget (PRINCIPLES.md → "Durable records").
...
Auto-dispatching a teammate? Read `skills/orchestrate/references/dispatch-templates.md` and fill the template matching the sub-issue's `safer:<modality>` label. Placeholders come from that file's schema only; a template that invents its own is malformed.

### Phase 5e — permission_request response protocol

Sweep surfaced a `permission_stall:` anomaly? Read `skills/orchestrate/references/permission-requests.md` and respond within the same tick. Leaving one blocked across ticks is a Forbidden-list violation.

### Phase 6 — Backtrack

Sub-task reported `ESCALATED`, `BLOCKED`, or `NEEDS_CONTEXT`? Read `skills/orchestrate/references/backtrack.md` and route by cause. Do not rescue it yourself; the blocked sub-task waits rather than being worked around.
...
1. **Under-specified intent.** The intent is so vague that decomposition would be guessing. → Create a `requirements` sub-task first; do not attempt further decomposition until the contract is `done`. Status: `NEEDS_CONTEXT`.
...
6. **Three diagnose splits without convergence.** When the parent epic's `## Diagnose splits (count: N)` section reaches 3 and no fork has returned codex `confirmed-root-cause`, the bug surface is wider than diagnose can map. Park the active diagnose sub-issues at `awaiting-amendment`. Escalate-target: `/safer:requirements` (the symptom needs re-statement) or `/safer:architect` (the failure is structural, not localized). Quote the codex verdicts from each fork in the stop-the-line comment; the user picks the next modality.
