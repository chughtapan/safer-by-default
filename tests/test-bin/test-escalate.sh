...
test_requires_args() {
  ...
  "$BIN" --from requirements >/dev/null 2>&1; rc=$?
  assert_nonzero "$rc" "missing --to --cause should fail"
}

test_validates_status() {
  local rc
  "$BIN" --from requirements --to architect --cause x --status INVALID >/dev/null 2>&1; rc=$?
  assert_nonzero "$rc" "invalid --status should fail"
}

test_validates_confidence() {
  local rc
  "$BIN" --from requirements --to architect --cause x --confidence MEDIUM >/dev/null 2>&1; rc=$?
