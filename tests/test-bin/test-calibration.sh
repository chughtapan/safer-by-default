...
seed_state() {
  ...
  for _ in 1 2 3 4 5; do
    SAFER_STATE_DIR="$state" "$LOG" --event-type safer.skill_run --modality requirements --session t-$RANDOM >/dev/null 2>&1
  done
  SAFER_STATE_DIR="$state" "$LOG" --event-type safer.escalation_triggered --modality requirements --session t-$RANDOM >/dev/null 2>&1
...
test_tags_healthy() {
  ...
  out=$(SAFER_STATE_DIR="$state" "$BIN" 7d 2>&1)
  rm -rf "$state"
  assert_contains "$out" "requirements" "requirements row present"
