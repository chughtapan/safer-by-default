...
**Autonomy budget.**
- Dispatch `/safer:diagnose`, `/safer:implement-junior`, `/safer:review-senior`, `/safer:verify` in that order.
- Open draft PRs.
- Run lint, typecheck, tests; iterate until green.
- Merge after peer review green AND CI green AND verify SHIP.

**Always-park.**
- Ratchet-up (diagnose→spec, diagnose→architect, implement-junior→architect): the original assumption was a routine fix; ratchet means it isn't.
- Force-push, branch deletion, infra/CI/deploy edits.
- Adding new sub-issues for follow-up bugs surfaced during diagnosis.
