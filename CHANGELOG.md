...
- **Test CI.** `.github/workflows/tests.yml` runs `bin/safer-gen-skills --check` and `tests/run-tests.sh` on every push and pull request. `AGENTS.md` has asserted since it was created (at `5c19e08`, when `VERSION` read `0.1.3`) that CI fails on a stale `SKILL.md`; until now no workflow ran either command, so a hand-edited `SKILL.md` or an un-rendered `.tmpl` edit could ship doctrine that disagreed with itself.
- **`PRINCIPLES.md` → "Composing with gstack"**, in Part 2 (Discipline). Carries the precedence rule (safer wins on scope, gstack ETHOS wins on quality-within-scope) and the hold-scope rule for user-prompting gstack skills. `AGENTS.md` has cited this heading by name since `5c19e08` and it did not exist. It belongs in Discipline because it is a scope rule: an agent that lets a composed reviewer widen its budget has violated Principle 6 regardless of how good the review was.
- **`./setup` now checks gstack**, the one hard dependency it never verified, and both entry points now use the same predicate. Two install shapes count: the `~/.claude/skills/gstack/` directory, or an enabled `gstack@*` plugin in `settings.json` or `settings.local.json`. `/safer:setup` previously tested only the directory, so it rejected a marketplace-plugin install that `./setup` accepts. A user with that shape passed one entry point and failed the other with nothing explaining why.
- **`tests/test-bin/test-doc-citations.sh`**, which pins the defect class `--check` structurally cannot see. The stale-render check compares a template against its render; it never reads prose, so it passes while a doc cites a heading nobody wrote. Two checks: relative markdown links resolve, and that a citation naming a `PRINCIPLES.md` heading resolves to one that exists. Its controls plant the real defect shapes rather than synthetic ones, because this checker has twice reported green on the defect it was written for: v1 matched only the bare-filename citation form and so never saw the markdown-link form `AGENTS.md` actually uses, and v2 compared with `grep -qF`, a substring match, so renaming a heading to `<original> RENAMED` still matched. Both are now controls. Verified by re-planting all three historical defects (renamed heading, dangling citation, dead link) and confirming each fails.

  Two things the check found immediately, neither of which any prior gate could see: `skills/orchestrate/SKILL.tmpl` cited `PRINCIPLES.md` -> Durability (the heading is "Durable records") and `skills/orchestrate/references/auto-monitor.md` cited "Process issues are first-class artifacts" (the heading is "Every output carries receipts"). Both fixed.
...
- **`ARCHITECTURE.md` update-gate list named three skills wrongly.** It listed `contract`, `contract-init`, `contract-migrate`; the plain gate lives on `requirements`, `architect`, `diagnose`, `spike`, `research`, `setup`, and `ux-audit`. `orchestrate` carries it with different semantics (fresh-pipeline starts only), so it is described separately rather than listed with the rest. `spec-init` and `spec-migrate` never carried it at all, so the doc claimed a gate on two skills that never had one.
...
- **Two calibration scenarios asserted a routing target that doctrine no longer uses.** `scenarios/living-spec/exit-code-11-routing.yaml` and `stale-sidecar-detected.yaml` named `/safer:contract` as the PASS criterion while `skills/verify/SKILL.tmpl:287,291` routes to `/safer:requirements`. The suite would have failed an agent that behaved correctly. Missed by the 0.5.0 rename sweep.
- **`scenarios/README.md` was half-fixed.** Removing the dead spec link orphaned four citations to it ("spec §7", "spec open question 2", "spec open question 3"), and the replacement sentence claimed those rules lived in the README when they did not. Citations dropped; the rules they pointed at are stated in the file.
- **The plugin-enabled check read only `settings.json`** while the gstack check twelve lines above read both it and `settings.local.json`. A plugin enabled only in the local file is still enabled. Both now share one predicate.
- **Four sentence fragments left by the em-dash purge in `682daee`** at `README.md:7` and `PRINCIPLES.md:301`, where a removed em-dash left a bare ` , `.
